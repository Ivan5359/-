...
const MIN_SCAN_GAP_MS = Number(process.env.MIN_SCAN_GAP_MS || 45 * 1000);
const MAX_SCAN_LIMIT = Math.min(50, Math.max(1, Number(process.env.MAX_SCAN_LIMIT || 25)));
...
function inferCategory(text = "") {
  ...
  return ["roofing", "hvac", "painting", "flooring", "fencing", "landscaping", "remodeling", "plumbing"]
    .find((category) => value.includes(category)) || "local contractor";
}

function normalizePhone(phone = "") {
  const digits = String(phone).replace(/[^\d+]/g, "");
  return digits.length >= 10 ? clean(phone) : "";
}

function defaultNextAction(temperature) {
  if (temperature === "Hot") return "Проверить черновик и написать сегодня";
  if (temperature === "Warm") return "Проверить пробелы в заявке и поставить follow-up";
  return "Оставить в дожим и найти недостающие контакты";
}
...
function enforceScanWindow() {
  const elapsed = Date.now() - lastScanStartedAt;
  if (lastScanStartedAt && elapsed < MIN_SCAN_GAP_MS) {
    const retryAfter = Math.ceil((MIN_SCAN_GAP_MS - elapsed) / 1000);
    const error = new Error(`Scan rate limit active. Retry after ${retryAfter}s.`);
    error.status = 429;
...
function normalizeLead(input = {}) {
  const businessName = clean(input.businessName || input.business_name || input.name || input.title) || "Unnamed contractor";
  const category = clean(input.category || inferCategory(`${businessName} ${input.type || ""}`));
  ...
  const score = clampScore(input.score || scoreLead({ ...input, businessName, category, city, state, website, phone, email }));
  ...
  return {
    ...
    followUpMessage: clean(input.followUpMessage || input.follow_up_message) || buildFollowUpMessage({ name: businessName, category, temperature }),
...
async function migratePostgres() {
  ...
  `);
  const existing = await pool.query("select key from app_settings where key = $1", ["autopilot"]);
  if (!existing.rowCount) {
    await pool.query("insert into app_settings (key, value) values ($1, $2)", ["autopilot", defaultSettings().autopilot]);
  }
}
...
async function nominatimSearch({ query, city, limit = 20 }) {
  ...
  url.searchParams.set("q", [query, city].filter(Boolean).join(" in ") || "roofing contractor");
...
function leadFromPlace(place, params = {}) {
  ...
  return normalizeLead({
    ...
    city: address.city || address.town || address.village || address.county || params.city || "",
    state: address.state || "",
    source: "nominatim",
    website: place.extratags?.website || place.extratags?.contact_website || "",
    phone: normalizePhone(place.extratags?.phone || place.extratags?.contact_phone || ""),
    email: place.extratags?.email || place.extratags?.contact_email || "",
    raw: place
  });
}
...
async function runScan(params = {}) {
  ...
  const limit = Math.min(Math.max(1, Number(params.limit || params.dailyLimit || autopilot.dailyLimit || 20)), MAX_SCAN_LIMIT);
...
const extractEmails = (html) => [...new Set((html.match(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi) || []).map((email) => email.toLowerCase()))].slice(0, 5);
const extractPhones = (html) => [...new Set((html.match(/(?:\+?1[\s.-]?)?(?:\(?\d{3}\)?[\s.-]?)\d{3}[\s.-]?\d{4}/g) || []).map((phone) => phone.trim()))].slice(0, 5);
const extractTitle = (html) => clean((html.match(/<title[^>]*>([^<]+)<\/title>/i) || [])[1]).slice(0, 120);
...
async function deepCheckLead(lead) {
  ...
  if (!website) {
    return {
      ...lead,
      lastChecked: nowIso(),
      notes: [lead.notes, "Deep Check: сайт пока не указан."].filter(Boolean).join("\n")
    };
  }
  ...
  try {
    ...
    return {
      ...
      notes: [
        lead.notes,
        `Deep Check: ${response.ok ? "сайт доступен" : `сайт вернул ${response.status}`}${title ? `, title "${title}"` : ""}.`
      ].filter(Boolean).join("\n")
    };
  } catch (error) {
    return {
      ...lead,
      lastChecked: nowIso(),
      notes: [lead.notes, `Deep Check: проверка сайта не удалась (${error.message}).`].filter(Boolean).join("\n")
    };
  }
}

function leadsToCsv(leads) {
  const headers = ["businessName", "category", "city", "state", "temperature", "score", "status", "website", "phone", "email", "nextAction", "followUpDue", "followUpMessage", "notes"];
  const escape = (value) => `"${String(value ?? "").replace(/"/g, '""')}"`;
  return [headers.join(","), ...leads.map((lead) => headers.map((key) => escape(lead[key])).join(","))].join("\n");
}
...
async function configureAutopilot(input = {}) {
  ...
  const next = {
    ...
    dailyLimit: Math.min(MAX_SCAN_LIMIT, Math.max(1, Number(input.dailyLimit || current.dailyLimit || 25))),
...
app.post("/api/autopilot/start", async (req, res, next) => {
  try {
    res.json({ autopilot: await configureAutopilot({ ...req.body, enabled: true }) });
  } catch (error) {
    next(error);
  }
});
...
app.post("/api/autopilot/stop", async (_req, res, next) => {
  try {
    const current = await getSetting("autopilot");
    res.json({ autopilot: await configureAutopilot({ ...current, enabled: false }) });
  } catch (error) {
    next(error);
  }
});
...
app.post("/api/deep-check", async (req, res, next) => {
  try {
    ...
    const checked = [];
    for (const lead of targets) checked.push(await upsertLead(await deepCheckLead(lead)));
    res.json({ checked });
  } catch (error) {
    next(error);
  }
...
app.use((error, _req, res, _next) => {
  console.error(error);
  if (error.status === 429) {
    res.setHeader("Retry-After", String(error.retryAfter || 45));
  }
  res.status(error.status || 500).json({
    error: "RevenueSprint server error",
    detail: process.env.NODE_ENV === "production" && !error.status ? "Проверь Railway logs." : error.message
  });
});
...
app.listen(PORT, () => {
  console.log(`RevenueSprint 48 работает на порту ${PORT}, хранилище: ${storageMode}`);
});
